...
def plot_delays_by_airline():
    """
    Generate and display a bar chart showing the percentage of delayed flights by airline.

    A flight is considered delayed if the departure delay exceeds 20 minutes.
    Data is aggregated from the flights and airlines tables in the database.
    """
...
def plot_delays_by_hour():
    """
    Generate and display a bar chart showing the percentage of delayed flights by hour of the day.

    The hour is extracted from the scheduled departure time.
    Flights are considered delayed if the departure delay exceeds 20 minutes.
    """
    engine = create_engine('sqlite:///flights.sqlite3')
...
def plot_heatmap_of_routes():
    """
    Generate and display a heatmap showing the percentage of delayed flights between origin and destination airports.

    Each cell represents a flight route, colored by the proportion of delayed flights (>20 min delay) on that route.
    """
    engine = create_engine('sqlite:///flights.sqlite3')
...
def plot_map_of_routes():
    """
    Generate and display an interactive map visualizing the major delayed flight routes across the USA.

    Only routes with more than 30% delayed flights are drawn.
    Routes are color-coded: orange for moderate delays and red for heavy delays.
    The map is saved as 'delayed_routes_map.html' and automatically opened in the default browser.
    """
    engine = create_engine('sqlite:///flights.sqlite3')
